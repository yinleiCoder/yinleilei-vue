        ...
        <el-row :gutter="20">
            <el-col :span="12" :offset="6">
                <div class="video-data">
                    <span class="cusSpan">{{video.created_at | moment("YYYY-MM-DD h:mm:ss") }}</span>投稿 -
                    <span class="cusSpan">{{video.view}}</span>人已浏览过
                </div>
            </el-col>
        </el-row>
        <el-row :gutter="20">
...
export default {
    // 用的是video.js播放视频，配合vue就需要vue-video-player这个模块
    name: 'showVideo',
    components: {
        videoPlayer
    },
    data() {
        return {
                ...
                playerOptions: {
                    ...
                    sources: [{
                        type: "video/mp4",
                        // src: "https://giligili-yinlei.oss-cn-shanghai.aliyuncs.com/miku-web.mp4"
                        // src: "https://giligili-yinlei.oss-cn-shanghai.aliyuncs.com/miku-web.mp4"
                        src: '',
                    }],
                },
    ...
    methods: {
        load(){
            API.getVideo(this.$route.params.videoID).then(res =>{
                this.video = res.data; 
                console.log(this.video.url.endsWith('mp4'));
...
<style lang="scss" scoped>
    ...
    #boards {
        ...
        #building-smoke {
            animation: 2s linear 1s building-smoke alternate
            infinite;
        }
